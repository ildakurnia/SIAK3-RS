...
export const Login: React.FC<LoginProps> = ({ onLoginSuccess }) => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-sky-950 to-slate-900 flex items-center justify-center p-4 font-['Plus_Jakarta_Sans',sans-serif]">
      ...
      <div className="max-w-md w-full relative z-10 space-y-6 animate-scaleUp">
        
        {/* Brand Header */}
        <div className="text-center space-y-2">
          <div className="w-16 h-16 rounded-2xl medical-gradient flex items-center justify-center text-white mx-auto shadow-xl shadow-sky-500/25 border border-white/20">
            <ShieldCheck className="w-9 h-9" />
          </div>
          <h1 className="text-2xl sm:text-3xl font-extrabold text-white tracking-tight">SIAK3-RS</h1>
          <p className="text-xs text-sky-200/90 font-medium">Sistem Informasi Audit Keselamatan & Kesehatan Kerja Rumah Sakit</p>
        </div>
        ...
        <div className="bg-white/95 backdrop-blur-md rounded-3xl p-6 sm:p-8 shadow-2xl border border-white/20 space-y-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="w-full py-3.5 px-6 bg-gradient-to-r from-sky-600 to-blue-700 hover:from-sky-700 hover:to-blue-800 text-white text-sm font-extrabold rounded-xl shadow-lg shadow-sky-600/25 transition-all flex items-center justify-center space-x-2 disabled:opacity-70"
            >
              {isLoading ? (
